...
@Composable
private fun ResizableAsyncImage(entry: EducationEntry) {
    val context = LocalContext.current
    var imageAspectRatio by remember { mutableStateOf<Float?>(null) }
    val imageRequestBuilder = remember(entry.imageUrl) {
        getDefaultImageRequestBuilder(context, entry.imageUrl)
            .listener(
                onSuccess = { _, result ->
